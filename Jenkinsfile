		...
		stage("Checkout GIT") {
			steps {
				script {
					...
					env.SUM_FILE = 'pifield_lite_arm64.sha256sum'
				}
			}
		}
		stage("Upload") {
			steps {
				sh 'curl -sS -T ${TEMP_DIR}/${RELEASE_NAME}.img.xz -u ${FTP_CREDENTIALS_USR}:${FTP_CREDENTIALS_PSW} ftp://${FTP_IP_CREDENTIALS}/data/pi-field/'
			}
		}
		stage("Check") {
			steps {
