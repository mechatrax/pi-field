pipeline{
			...
			steps {
				withCredentials([usernamePassword(credentialsId: '5b25baba-433c-41a9-b104-59e49ec74e49', passwordVariable: 'JENKINS_TOKEN', usernameVariable: 'JENKINS_USERNAME')]){
					script {
						...
						env.SUM_FILE = 'pifield_lite_armhf.sha256sum'
					}
				}
			}
		}
	...
	post {
		success {
			hangoutsNotify message: "✔ Pi-field の SD イメージ ${RELEASE_NAME} のリリースに成功しました", token: "${POST_CREDS}", threadByJob: false
		}
		failure {
			hangoutsNotify message: "❌ Pi-field の SD イメージ ${RELEASE_NAME} のリリースに失敗しました", token: "${POST_CREDS}", threadByJob: false
		}
	}
}
